...
%   2. act-T-H means: "task T is active at hour H"
...
defineTH(Time):-
  ...
  End is Hour+D-1,
  between(Hour,End,H),
  writeClause([\+start-T-Hour, act-T-H]),  %% start-T-Hour --> act-T-H <==> (\+start-T-Hour or act-T-H)
  fail.
defineTH(_).
    
eachTaskStartsOnce(Time):-
  task(T), findall(start-T-H, availableHour(H,T,Time), Lits),
  exactly(1,Lits), fail.
